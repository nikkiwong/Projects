    ...
    return <div className="page">
        <div className="container">
            <div className="auth-container">
                {user && <p>User logged in: {user.email}</p> && <button onClick={() => logout()}>Logout</button>}
                ...
                {mode === "signup" ? (<p>Already have an account? <span className="auth-link" onClick={() => { setMode("login"); setError(null)  }}>Login</span></p>) : (<p>Don't have an account? <span className="auth-link" onClick={() => { setMode("signup"); setError(null) }}>Sign Up</span></p>)}
            </div>
        </div>
    </div>
}